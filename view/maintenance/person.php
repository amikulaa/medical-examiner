 ...
 <body>
	<div class="container">
		<div id="table-view" class="col-sm-10">
    			...
    			<div id="maintenance-editing">
    				...
    				<div class='add_row_container' id='add_row_container'>
    					<row><form action="" method="POST">
                    		<input name='add_row_value' id='add_row_value' type='text' placeholder='Person' required/>
    				...
    				<div class='update_row_container' id='update_row_container'>
        				<row><form action="" method="POST">
            				...
            				<input name='update_row_value' id='update_row_value' type='text' placeholder='Update Person' required/>
            ...
            <!-- Load View echo $this->entry_model->fill_maintenence_view("person", "name", "");?> -->
				<?php if(isset($_POST['search'])){
    				    $search_filter = $_POST['search'];
    				    $column_headers = ["person_name"];
    				    echo $this->entry_model->fill_maintenance_view("person", $search_filter, $column_headers);
				    } else if(isset($_POST['show-all-button'])) {
    				    echo $this->entry_model->fill_maintenance_view("person", "", "");
    				} else if(isset($_POST['add_row_value'])){
    				    $data = $_POST['add_row_value'];
    				    $add_vals = ["person_name" => $data];
    				    $this->entry_model->add_row('person', $add_vals);
    				    echo $this->entry_model->fill_maintenance_view("person", "", "");
    				    ...
    				    $updated_vals = ["person_name" => $updated_data];
    				    $this->entry_model->update_row('person', $row_id, $updated_vals);
    				    echo $this->entry_model->fill_maintenance_view("person", "", "");
    				} else if(isset($_POST['delete_row_id'])){
    				    $row_id = $_POST['delete_row_id'];
    				    $this->entry_model->delete_row('person', $row_id);
    				    echo $this->entry_model->fill_maintenance_view("person", "", "");
    				} else {
    				    echo $this->entry_model->fill_maintenance_view("person", "", "");
    				}
