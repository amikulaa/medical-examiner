<!-- view/maintenance/citystatezip.php -->
 <body>
	<div class="container">
		<div id="table-view" class="col-sm-10">
        	<h2><b>City, State Zip</b></h2>
        	<input id='table_name' value='citystatezip' hidden>
    		<hr/>
    			...
    			<br/>
    				<div class='add_row_container' id='add_row_container'>
    					<row><form action="" method="POST">
                    		<input name='add_row_value' id='add_row_value' type='text' placeholder='City, St Zip' required/>
    				...
    				<div class='update_row_container' id='update_row_container'>
        				<row><form action="" method="POST">
            				...
            				<input name='update_row_value' id='update_row_value' type='text' placeholder='Update City, St Zip' required/>
            ...
            <!-- Load View echo $this->entry_model->fill_maintenence_view("citystatezip", "name", "");?> -->
				<?php if(isset($_POST['search'])){
    				    ...
    				    $column_headers = ["citystatezip_name"];
    				    echo $this->entry_model->fill_maintenance_view("citystatezip", $search_filter, $column_headers);
    				} else if(isset($_POST['show-all-button'])) {
    				    echo $this->entry_model->fill_maintenance_view("citystatezip", "", "");
    				} else if(isset($_POST['add_row_value'])){
    				    $data = $_POST['add_row_value'];
    				    $add_vals = ["citystatezip_name" => $data];
    				    $this->entry_model->add_row('citystatezip', $add_vals);
    				    echo $this->entry_model->fill_maintenance_view("citystatezip", "", "");
    				    ...
    				    $updated_vals = ["citystatezip_name" => $updated_data];
    				    $this->entry_model->update_row('citystatezip', $row_id, $updated_vals);
    				    echo $this->entry_model->fill_maintenance_view("citystatezip", "", "");
    				} else if(isset($_POST['delete_row_id'])){
    				    $row_id = $_POST['delete_row_id'];
    				    $this->entry_model->delete_row('citystatezip', $row_id);
    				    echo $this->entry_model->fill_maintenance_view("citystatezip", "", "");
    				} else {
    				    echo $this->entry_model->fill_maintenance_view("citystatezip", "", "");
    				}
